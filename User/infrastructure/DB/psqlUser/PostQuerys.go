...
const SqlUpdatePostTitle = `UPDATE posts SET title = $1, updated_at = now() WHERE owner_email = $2`
...
const SqlGetAllPosts = `SELECT id, title, body, owner_email, created_at, updated_at FROM posts`
